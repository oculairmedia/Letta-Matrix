...
def _is_streaming_progress(text: str) -> bool:
    """Detect agent streaming progress messages that should not be re-processed.
    ...
    Also detects multi-line progress blocks (live-edit handler output)
    where every line is a progress indicator.
    """
    stripped = text.strip()
    if not stripped:
        return False
    ...
    # This handles both single-line ("🔧 Bash...") and multi-line blocks
    # ("🔧 Bash...\n✅ Bash\n🔧 Grep...") while rejecting mixed content
...
class _MessageCallbackRouter:
    ...
    def _should_skip_message(self, event, room_id) -> Optional[str]:
        ...
        # Skip streaming progress messages (🔧, ✅, ❌, ⏳) echoed back via bridge rooms.
        # These are generated by the agent's own streaming handler and must not be
        # re-processed, as they cause feedback loops in bridged rooms.
...
async def join_room_if_needed(client_instance, room_id_or_alias, logger: logging.Logger):
    ...
    try:
        ...
        if isinstance(response, JoinError):
            ...
            )  # Matrix error code like M_UNRECOGNIZED
