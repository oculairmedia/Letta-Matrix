...
docker logs matrix-tuwunel-deploy-matrix-client-1 2>&1 --tail 200 | grep "\[DEBUG\] AGENT ROUTING" | tail -5
echo ""

echo "5. Recent messages to any BMO-related room:"
docker logs matrix-tuwunel-deploy-matrix-client-1 2>&1 --tail 500 | grep -i "bmo\|tfSmwhqAWH3xZhN623" | tail -10
echo ""
...
echo ""
echo "docker logs matrix-tuwunel-deploy-matrix-client-1 --tail 50 | grep -A 5 -B 5 'Received message'"
echo ""
echo "Look for:"
