...
```
Matrix Rooms ←→ Bridge Service ←→ MCP Agent Mail Server
     ↓                                    ↓
...
Messages containing these keywords get forwarded Matrix → Agent Mail:
...
1. **Identity Mapper**: Maintains bidirectional Letta ↔ Matrix ↔ Agent Mail mappings
...
├── src/bridges/
│   ├── __init__.py
│   └── agent_mail_bridge.py           # Bridge service implementation
├── scripts/
│   ├── generate_identity_mapping.py   # Generate mapping from Matrix data
│   └── setup_agent_mail_bridge.sh     # Register bridge user in Matrix
├── docker/
│   └── Dockerfile.agent-mail-bridge   # Container image
├── matrix_client_data/
│   ├── agent_user_mappings.json       # Matrix agent data (source of truth)
│   └── agent_mail_mappings.json       # Generated identity mapping
